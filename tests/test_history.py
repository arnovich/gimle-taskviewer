...
def _commit(subject: str, paths=()) -> Commit:
    return Commit("abc", "erikarne", datetime(2026, 9, 24, tzinfo=timezone.utc), subject, tuple(paths))
...
def test_the_log_is_read_newest_first_with_merges_included(tmp_path: Path) -> None:
    ...
    git(repo, "merge", "--no-ff", "-m", "Merge pull request #1 from me/task/001_first", "task/001_first")
